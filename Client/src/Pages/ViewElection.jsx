  ...
  const handlePostLivenessAuth = useCallback(async (liveDescriptor) => {
    ...
    try {
      const verifyRes = await axios.post(serverLink + "verify-face", {
        ...
        descriptorsSequence: liveness.descriptors.slice(-15), // Send the specific frames for verification
      });
      ...
      try {
        if (webcamRef.current && webcamRef.current.video && webcamRef.current.video.readyState === 4) {
          ...
          if (detections) {
            ...
            const hasMinFrames = state.frameCount >= 15; // Increased for strict liveness
            const hasMovement = state.movedFrames >= 3;
            const hasBlink = state.blinkDetected;

            if (hasBlink && hasMovement && hasMinFrames) {
              const averaged = averageDescriptors(state.descriptors.slice(-15));
              if (averaged) {
              ...
              setBlinkStatus(`Collecting secure frames... ${state.frameCount}/15`);
            }
          ...
          <Grid item key={index} xs={12} sm={6} md={4}>
            <Card sx={{ 
              ...
              <CardMedia
                component="img"
                height="240"
                image={cand.avatar ? cand.avatar : facesLink + (cand.username || cand) + ".png"}
